...
| :-------------- | :---------------------------- | :-------------------------------------------------------- |
| Proxmox VE | Deploy and configure | Proxmox VE is an open-source server virtualization platform that supports both Kernel-based Virtual Machine (KVM) for full virtualization and Linux Containers (LXC) for lightweight virtualization. It features a web-based management interface, high availability clustering, software-defined storage, and disaster recovery tools, making it suitable for managing complex IT infrastructures efficiently. |
| OpenSearch Dashboard | Deploy, configure and integrate | OpenSearch Dashboards is an open‐source, browser-based analytics and visualization tool that works seamlessly with OpenSearch data. It can serve as a centralized platform to aggregate and visualize logs, metrics, and security events in real time. This not only helps you quickly identify anomalies and potential threats but also aids in fine-tuning your lab's configurations and performance. |
